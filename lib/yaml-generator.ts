...
export interface RestStepConfig {
  ...
  cookies?: string | Record<string, string>; // "auto" or key-value pairs
...
export interface ExpectConfig {
  ...
  jsonContains?: Record<string, any>;
  jsonPath?: Record<string, any>;
  performance?: {
    maxDuration?: number;
  };
...
export function generateYaml(suite: TestSuite): string {
  ...
  yamlObj.tests = suite.tests.map(step => {
    ...
    if (step.rest) stepObj.rest = cleanRestConfig(step.rest);
    ...
    // Add expect
    if (step.expect) {
      stepObj.expect = cleanExpectConfig(step.expect);
    }
...
function cleanExpectConfig(expect: ExpectConfig): any {
  const cleaned: any = {};

  if (expect.status) cleaned.status = expect.status;
  if (expect.body) cleaned.body = expect.body;
  if (expect.json) cleaned.json = expect.json;
  if (expect.performance) cleaned.performance = expect.performance;

  // Handle jsonContains: if it's a JSON string, parse it back to object
  if (expect.jsonContains) {
    if (typeof expect.jsonContains === 'string') {
      try {
        cleaned.jsonContains = JSON.parse(expect.jsonContains);
      } catch {
        cleaned.jsonContains = expect.jsonContains;
      }
    } else {
      cleaned.jsonContains = expect.jsonContains;
    }
  ...
  // Handle jsonPath: if it's a JSON string, parse it back to object
  if (expect.jsonPath) {
    if (typeof expect.jsonPath === 'string') {
      try {
        cleaned.jsonPath = JSON.parse(expect.jsonPath);
      } catch {
        cleaned.jsonPath = expect.jsonPath;
      }
    } else {
      cleaned.jsonPath = expect.jsonPath;
    }
...
function cleanRestConfig(rest: RestStepConfig): any {
  const cleaned: any = {};

  if (rest.url) cleaned.url = rest.url;
  if (rest.method) cleaned.method = rest.method;
  if (rest.headers && Object.keys(rest.headers).length > 0) cleaned.headers = rest.headers;
  if (rest.queryParams && Object.keys(rest.queryParams).length > 0) cleaned.queryParams = rest.queryParams;
  if (rest.multipart && Object.keys(rest.multipart).length > 0) cleaned.multipart = rest.multipart;

  // Handle body: if it's a JSON string, parse it back to object
  if (rest.body) {
    if (typeof rest.body === 'string') {
      try {
        // Try to parse as JSON
        cleaned.body = JSON.parse(rest.body);
      } catch {
        // If parsing fails, keep as string
        cleaned.body = rest.body;
      }
    } else {
      // Already an object
      cleaned.body = rest.body;
  ...
  // Handle cookies: keep "auto" as string, otherwise try to parse as JSON
  if (rest.cookies) {
    if (rest.cookies === 'auto') {
      cleaned.cookies = 'auto';
    } else if (typeof rest.cookies === 'string') {
      try {
        // Try to parse as JSON
        cleaned.cookies = JSON.parse(rest.cookies);
      } catch {
        // If parsing fails, keep as string
        cleaned.cookies = rest.cookies;
      }
    } else {
      // Already an object
      cleaned.cookies = rest.cookies;
