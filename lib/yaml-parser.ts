...
function parseTestStep(stepData: any): TestStep {
  ...
  if (stepData.rest) {
    // Convert body object to JSON string for form display
    let bodyValue = stepData.rest.body;
    if (bodyValue && typeof bodyValue === 'object') {
      bodyValue = JSON.stringify(bodyValue, null, 2);
    }

    // Handle cookies - keep as string if "auto", otherwise convert object to JSON
    let cookiesValue = stepData.rest.cookies;
    if (cookiesValue && typeof cookiesValue === 'object') {
      cookiesValue = JSON.stringify(cookiesValue, null, 2);
    }
  ...
  // Expect
  if (stepData.expect) {
    // Convert jsonContains and jsonPath objects to JSON strings for form display
    let jsonContainsValue = stepData.expect.jsonContains;
    if (jsonContainsValue && typeof jsonContainsValue === 'object') {
      jsonContainsValue = JSON.stringify(jsonContainsValue, null, 2);
    }

    let jsonPathValue = stepData.expect.jsonPath;
    if (jsonPathValue && typeof jsonPathValue === 'object') {
      jsonPathValue = JSON.stringify(jsonPathValue, null, 2);
    }
    ...
    step.expect = {
      ...
      jsonContains: jsonContainsValue,
